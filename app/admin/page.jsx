      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-xl text-gray-700">Gestiona el inventario</p>
          </div>
          <button
        ...
        {(isAdding || editingId !== null) && (
          <div className="bg-white rounded-lg shadow-lg p-8 mb-8 border-2 border-emerald-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm mb-2 text-gray-700">Subir imagen</label>
                <input
                  type="file"
                  accept="image/*"
                  onChange={(e) => {
              ...
              <div className="flex gap-4 pt-4">
                <button type="submit" className="flex-1 bg-emerald-700 text-white py-3 rounded flex items-center justify-center gap-2">
                  <Save className="w-5 h-5" /> {editingId !== null ? 'Guardar' : 'Agregar'}
                </button>
                <button type="button" onClick={handleCancel} className="px-6 bg-gray-300 text-gray-700 py-3 rounded">
                  Cancelar
                </button>
              </div>
            </form>
          ...
          <table className="w-full">
            ...
            <tbody>
              {products.map((product) => (
                <tr key={product.id} className="bg-white hover:bg-amber-50">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex justify-center gap-2">
                      <button onClick={() => handleEdit(product)} className="p-2 text-blue-600"><Edit2 className="w-5 h-5" /></button>
                      <button onClick={() => deleteProduct(product.id)} className="p-2 text-red-600"><Trash2 className="w-5 h-5" /></button>
                    </div>
                  </td>
                </tr>
              ))}
